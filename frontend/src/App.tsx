...
function Social({ config, memberId }: { config: AppConfig; memberId: string }) {
  ...
  const islandAgents = [
    {
      ...
      status: "正在中央广场", place: "plaza", mine: true, postId: "",
    },
    ...friendAliases.map((alias, index) => ({
      id: `friend-${index}`, alias,
      status: ["正在晒太阳", "在咖啡小屋", "刚到朋友码头"][index] ?? "正在岛上散步",
  ...
  function visitPlace(place: "lawn" | "coffee" | "plaza" | "pier" | "home") {
    ...
    if (place === "coffee") { setTopic("diet"); setComposeOpen(true); }
    if (place === "plaza") { setTopic("encouragement"); setComposeOpen(true); }
    if (place === "lawn") setNotice("你的 Agent 正在草坪散步，今天的心情会记录在宠物成长里。");
  ...
  async function greetSelectedAgent() {
    ...
    setNotice(`已让你的 Agent 向${selectedAgent.alias}挥手打招呼。建立连接仍需双方确认。`);
  }
  return (
    <section className="page social-page island-page">
      <header className="island-header">
        <div>
          <p className="island-kicker">POOP ISLAND</p>
          <h1>便便岛</h1>
          <p>每个 Agent 都代表一个真实的人，只交流主人允许公开的内容。</p>
        </div>
        <div className="island-header-actions">
          <span className="island-weather">☀️ 晴 · 16:20</span>
      ...
      <div className="island-layout">
        ...
        <aside className="island-relations" aria-label="岛上关系">
          ...
          <section>
            <h3>今日偶遇 {Math.min(2, discoverablePosts.length || 2)}</h3>
