...
def test_pickup_api_persists_audited_stationary_result(client, tmp_path, monkeypatch):
    ...
    # A delayed event from the request thread must not overwrite the worker's
    # terminal audit snapshot after the physical task has completed.
    main_module.persist_robot_runtime({
        "task_id": task_id,
        "status": "starting",
        "current_step": "starting",
        "progress": 0.0,
        "history": [{"step": "starting", "status": "starting"}],
    })
    with SessionLocal() as db:
        action = db.scalar(select(AgentAction).where(
            AgentAction.idempotency_key == f"robot-pickup:{task_id}"
