...
设备契约增加兼容的`data_kind`来源标记，首次并发重传由数据库约束与回滚后校验处理。新增一次性测试家庭/设备初始化脚本、迁移以及独立PostgreSQL持续集成检查；PostgreSQL驱动现在属于部署依赖。云库创建与最终切换仍待部署账号完成集成确认，当前不能据此宣称公网持久化完成。用法和验证边界见[测试数据迭代](../docs/releases/2026-09-21-hardware-test-data.md)。
