...
原上传、设备查询、认领和 `processing.llm_status=not_applicable` 保持不变。卡纸、橡皮泥只能得到演示说明，模型正文逐句检查，不能添加健康判断或缺失的传感器读数。多实例并发由数据库唯一约束和条件更新去重，失败或过期需手动重试；生成前后均复查家庭/成员权限。具体入口、实测记录和发布状态见[AI 演示解读验收](../docs/releases/2026-09-22-reader-ai-demo.md)。

## 硬件测试数据与持久化准备（2026-09-21）

当前Reader后端已连接独立Neon PostgreSQL测试库，迁移head为`b92f17c03a64`，自动建表和启动种子已关闭。手动采样接口严格保存相似度与量纲，设备密钥可查询不含成员资料的回执；Reader可连接独立测试家庭、认领及查看原始观测。原始采样不进入健康基线或模型报告。已用同一合成记录完成公网上传、浏览器认领和后端重新部署后读取。

兼容的`data_kind`来源标记、并发去重、一次性身份/设备初始化、驱动及真实PostgreSQL持续集成检查均已上线。PostgreSQL外键启用后发现并修复旧问答的父子记录写入顺序；一次真实DeepSeek问答恢复200。上传客户端和步骤见[Reader设备说明](scripts/README-reader-device.md)，完整验证与备份范围见[Reader公网联调发布](../docs/releases/2026-09-21-reader-hardware.md)。下方旧SQLite状态按历史日期理解，当前仍为demo且worker关闭。

## 当前部署模型（2026-09-21）

按用户要求，生产/预览环境与本地配置的六类文字任务统一指向 `deepseek-v4-pro`，生产后端已重新发布并完成两题公网验证。保留自动路由，通过任务配置覆盖分工；单次记录报告与后台动作仍使用 `policy`。本地已有进程需重启后读取配置，百川接入实现和历史消息来源保留。下方百川分工、测试及参数均按其历史日期理解，详见[本次切换与验证](../docs/releases/2026-09-21-deepseek.md)。
...
传入不存在的`conversation_id`返回409/`CONVERSATION_EXPIRED`，在写入消息、报告或模型调用前结束，不静默新建；明确开始新对话时不传旧ID。健康知识提到家庭成员不触发家庭管理工具，真正的管理请求和红线优先级由共享意图规则判定，权限与确认流程保持原有约束。
