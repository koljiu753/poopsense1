...
_robot_audit_lock = threading.Lock()


def _robot_snapshot_rank(snapshot: dict) -> tuple[int, int, float]:
    """Order runtime events so a delayed early event cannot replace newer audit state."""
    terminal = snapshot.get("status") in {"completed", "failed", "stopped"}
    history = snapshot.get("history")
    history_length = len(history) if isinstance(history, list) else 0
    progress = snapshot.get("progress")
    numeric_progress = float(progress) if isinstance(progress, (int, float)) else 0.0
    return (2 if terminal else 1, history_length, numeric_progress)
...
def persist_robot_runtime(snapshot: dict) -> None:
    ...
    if not task_id:
        return
    # Notifications can arrive from the request thread and the robot worker thread.
    # Serialize and compare them so an older `starting` snapshot cannot commit after
    # `pickup_completed` and overwrite the final audit result.
    with _robot_audit_lock:
        with SessionLocal() as db:
            ...
            if not action:
                return
            existing = action.result if isinstance(action.result, dict) else {}
            if existing and _robot_snapshot_rank(snapshot) < _robot_snapshot_rank(existing):
                return
