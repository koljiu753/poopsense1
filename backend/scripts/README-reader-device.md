...
2026-09-22 软件自动流程补充：上传程序、上传地址、JSON 和密钥保持本说明原约定。演示时预先打开 [Reader 硬件演示](https://poopsense-reader-0912.vercel.app/#/demo)，连接对应测试家庭并选好设备；上传后网页自动展示颜色/形状卡通并调用 DeepSeek 生成演示解读，不必每次查 ID 或点生成。未认领记录保持未认领，卡纸/橡皮泥不进入个人健康报告。详见[本轮操作与验收](../../docs/releases/2026-09-22-reader-auto-demo.md)。下方 9/21 验收和原交付 ZIP 是历史快照；上传工具仍原样可用，Exhibition 备用包保留。
