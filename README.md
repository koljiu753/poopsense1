...
2026-09-21硬件测试数据迭代：明确模拟/硬件测试来源，待认领自动刷新，补齐并发上传去重、测试家庭初始化与PostgreSQL验证。云数据库尚待账号完成集成确认，当前公网仍不能保证记录跨实例保存，详见[本轮实现与接库边界](docs/releases/2026-09-21-hardware-test-data.md)。
...
直接问答时新记录只显示提醒，不抢走正在阅读的回答。演示会话失效时保留原问题和草稿，用户点击“在新对话中重试”后才重新发送；不能保证临时数据库恢复前文。
...
本站仍使用共用虚构演示数据和临时SQLite，worker关闭，记录可能因实例切换或重启丢失。请使用虚构内容体验。独立身份、持久数据库和正式生产运行尚未完成；模型可能扩展事实或产品能力，来源需要核验。
