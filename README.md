...
[打开新版 PoopSense](https://poopsense-mobile.vercel.app/) · [本次发布说明](docs/releases/2026-09-12-chat.md)

2026-09-12 已发布手机问答体验更新，通过同域代理连接现有公网演示后端，不依赖本地电脑。输入框位于导航上方，回答支持段落、列表和重点；回复到达后立即允许追问，可选详情不再阻挡发送。新后端的短答策略与模型参数调整尚未部署到这个演示入口，不能据此宣称模型生成已经提速。真实问答检查与已验证范围见本次发布说明。
...
2026-09-12 本地前端 **96/96 通过**、后端 **116/116 通过**，TypeScript/Vite 构建通过。自动化使用隔离环境，不调用真实模型或硬件；[代码提交的 GitHub Actions](https://github.com/koljiu753/poopsense1/actions/runs/34701842589) 也已成功。
